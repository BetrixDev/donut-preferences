...
export function DonutTracker() {
  ...
  const [selectedFriend, setSelectedFriend] = useState<Id<"friends"> | null>(null);
  ...
  const seedDonuts = useMutation(api.donuts.seed);

  // Seed donuts on first load
  useEffect(() => {
    if (donuts.length === 0) {
      seedDonuts({}).catch(console.error);
    }
  }, [donuts.length, seedDonuts]);
  ...
  const handleDonutSelection = async (friendId: Id<"friends">, donutName: string) => {
    try {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-white rounded-lg shadow-md p-6">
        <h2 className="text-2xl font-semibold text-gray-800 mb-4">Add a Friend</h2>
      ...
      {/* Friends List */}
      <div className="bg-white rounded-lg shadow-md p-6">
        <h2 className="text-2xl font-semibold text-gray-800 mb-6">Friends & Their Donut Preferences</h2>
        
        {friends.length === 0 ? (
          <p className="text-gray-500 text-center py-8">
            No friends added yet. Add some friends to track their donut preferences!
          </p>
        ) : (
            ...
            {friends.map((friend) => (
              <div key={friend._id} className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
                ...
                {friend.favoriteDonut ? (
                  <div className="mb-3">
                    <p className="text-sm text-gray-600 mb-1">Favorite Donut:</p>
                    <div className="flex items-center gap-2 p-2 bg-pink-50 rounded-lg">
                      <span className="text-2xl">
                        {donuts.find(d => d.name === friend.favoriteDonut)?.imageUrl || "🍩"}
                      </span>
                      <span className="font-medium text-pink-700">{friend.favoriteDonut}</span>
                    </div>
                  </div>
                ) : (
                  <p className="text-gray-500 text-sm mb-3">No donut preference set</p>
                )}
                
                <button
                  onClick={() => setSelectedFriend(selectedFriend === friend._id ? null : friend._id)}
                  ...
                  <div className="mt-3 grid grid-cols-2 gap-2">
                    {donuts.map((donut) => (
                      <button
                        key={donut._id}
                        onClick={() => {
                          handleDonutSelection(friend._id, donut.name);
                      ...
                      >
                        <span className="text-lg">{donut.imageUrl}</span>
      ...
      {/* Donut Reference */}
      <div className="bg-white rounded-lg shadow-md p-6">
        <h2 className="text-2xl font-semibold text-gray-800 mb-6">Available Donuts</h2>
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-4">
          {donuts.map((donut) => (
            <div key={donut._id} className="text-center p-3 border border-gray-200 rounded-lg hover:shadow-md transition-shadow">
              <div className="text-3xl mb-2">{donut.imageUrl}</div>
              <div className="text-sm font-medium text-gray-800">{donut.name}</div>
              <div className="text-xs text-gray-500 mt-1">{donut.description}</div>
            </div>
          ))}
        </div>
      </div>
